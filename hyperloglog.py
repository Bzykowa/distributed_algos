...
def hyperLogLog(M_set: Multiset, b: int, h: Hash32Bit) -> float:
    ...
    E = alpha * (m ** 2) * (sum([2 ** (-i) for i in M[1:]]) ** (-1))

    # Corrections (relative error +- 1.04/sqrt(m) from 2007 paper)
    if E <= 2.5*m:
        V = len(list(filter(lambda zero: zero == 0, M[1:])))
        if V > 0:
...
def graphBs(x_axis: list, y_axis: list, save: bool = False) -> None:
    plt.scatter(x=x_axis, y=y_axis[0], c='red', label='m=2^4')
    plt.scatter(x=x_axis, y=y_axis[1], c='yellow', label='m=2^8')
    plt.scatter(x=x_axis, y=y_axis[2], c='green', label='m=2^12')
    plt.scatter(x=x_axis, y=y_axis[3], c='cyan', label='m=2^16')
    plt.xlabel('n')
